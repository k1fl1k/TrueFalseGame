<?php

use k1fl1k\truefalsegame\Livewire\Forms\LoginForm;
use Illuminate\Support\Facades\Session;
use Livewire\Attributes\Layout;
use Livewire\Volt\Component;

new #[Layout('layouts.guest')] class extends Component {
    public LoginForm $form;

    public function login(): void
    {
        $this->validate();

        $this->form->authenticate();

        Session::regenerate();

        $this->redirectIntended(default: route('truth-or-lie.gamehub', absolute: false), navigate: true);
    }
};
?>

<div class="flex h-screen w-full overflow-hidden font-sans bg-[#1a1a1a] text-[#f5f5f5]">
    <!-- Left side with background image -->
    <div class="left" style="background-image: url('{{ asset('images/truefalse.png') }}');"></div>

    <!-- Right login panel -->
    <div class="right flex flex-col justify-center items-center w-full px-6">
        <div class="logo mb-2">TrueFalse</div>
        <div class="subtitle mb-6">Colorize your day with pretty arts</div>

        <!-- Session Status -->
        <x-auth-session-status class="mb-4" :status="session('status')" />

        <form wire:submit="login" class="w-full max-w-md">
            <!-- Email Address -->
            <div class="mb-4">
                <x-input-label for="email" :value="__('Email')" />
                <x-text-input wire:model="form.email" id="email" class="block mt-1 w-full" type="email"
                              name="email" required autofocus autocomplete="username" />
                <x-input-error :messages="$errors->get('form.email')" class="mt-2" />
            </div>

            <!-- Password -->
            <div class="mb-4">
                <x-input-label for="password" :value="__('Password')" />
                <x-text-input wire:model="form.password" id="password" class="block mt-1 w-full" type="password"
                              name="password" required autocomplete="current-password" />
                <x-input-error :messages="$errors->get('form.password')" class="mt-2" />
            </div>

            <!-- Remember Me -->
            <div class="mb-6">
                <label for="remember" class="inline-flex items-center">
                    <input wire:model="form.remember" id="remember" type="checkbox"
                           class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                           name="remember">
                    <span class="ms-2 text-sm text-gray-300">{{ __('Remember me') }}</span>
                </label>
            </div>

            <!-- Centered buttons -->
            <div class="flex flex-col gap-3 items-center">
                <button class="button btn-gold w-full">{{ __('Log in') }}</button>

                @if (Route::has('register'))
                    <a href="{{ route('register') }}" class="button btn-dark w-full text-center">{{ __('Create an account') }}</a>
                @endif

                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}"
                       class="note text-center hover:underline"
                       wire:navigate>
                        {{ __('Forgot your password?') }}
                    </a>
                @endif
            </div>
        </form>
    </div>
</div>
